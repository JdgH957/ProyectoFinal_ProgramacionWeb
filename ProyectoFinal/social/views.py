...
def informacion(request,codigo):
    ...
    contexto = {"emprendimiento":emprendimiento,"comentarios":comentarios, "productos":productos}
...
def ircrearproducto(request,codigo):
    emprendimiento = Emprendimiento.objects.get(codigo=codigo)
    return render(request, "crearproducto.html",{"emprendimiento":emprendimiento})

def crearproducto(request,id):
    nombre = request.POST['nombreproducto']
    decripcion = request.POST['descripcionproducto']
    precio = request.POST['precioproducto']
    emprendimiento = Emprendimiento.objects.get(codigo=id)
    idcliente=emprendimiento.cliente.codigo

    producto=Producto.objects.create(nombre=nombre, decripcion=decripcion, tipo=precio, emprendimiento=emprendimiento)
    return redirect('../perfil/'+str(idcliente))
...
def eliminarEmprendimiento(request,codigo):
    emprendimiento = Emprendimiento.objects.get(codigo=codigo)
    url = emprendimiento.cliente.codigo
    emprendimiento.delete()
    return redirect('../perfil/'+ str(url))
